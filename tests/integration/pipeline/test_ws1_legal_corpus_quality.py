...
def _mitigated_risk_assertions(case: dict, gates: dict[str, dict]) -> None:
    """按 fixture 中的 risk_codes 做额外针对性断言。"""
    risk_codes = set(case.get("risk_codes") or [])
    if "article_cross_contamination" in risk_codes:
        measured = gates["article_numbers"]["measured"]
        assert measured.get("duplicate_numbers") in (None, []), "发现重复条号"
        assert measured.get("article_count") == case["expected_article_count"]
    if "word_page_field" in risk_codes:
        assert gates["body_purity"]["measured"].get("contaminant_count", 0) == 0
    if "trailing_print_info" in risk_codes:
        assert gates["body_purity"]["measured"].get("contaminant_count", 0) == 0
    if "chunk_structure_overlap" in risk_codes:
        measured = gates["chunk_coverage"]["measured"]
        assert measured.get("out_of_range_article_indexes") in (None, [])
        assert measured.get("missing_article_indexes") in (None, [])


@pytest.mark.parametrize("case", _load_ws1_cases(), ids=lambda c: c["relative_path"])
def test_ws1_legal_corpus_quality_passes(case: dict, tmp_path: Path) -> None:
    ...
    run_id = f"ws1-quality-{sha256(case['relative_path'].encode('utf-8')).hexdigest()[:12]}"
    ...
    # 自动通过的报告中不应存在未解释的 warning
    assert all(gate["outcome"] == "pass" for gate in quality_report["gates"])

    _mitigated_risk_assertions(case, gates)


def test_ws1_scope_has_exactly_eight_cases() -> None:
    cases = _load_ws1_cases()
    assert len(cases) == 8
    assert all(
        set(item)
        == {
