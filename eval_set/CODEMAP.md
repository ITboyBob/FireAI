...
| --- | --- | --- |
| `RAG_Golden_Set.json` | Golden Set 数据 | 结构化正例 QA 集（`records[]`，含 `qa_id`/`question`/`answer`），是 Adversarial Case 改写与 Edge Case 生成的输入语料。 |
...
| `Edge_Case生成设计方案.md` | Edge Case 设计方案 | Draft；采纳 Salesforce Unanswerability_RAGE 论文分类的五个二级 Bucket（Underspecified / Out-of-Database / Nonsensical / False-presupposition / Modality-limited），`context` 字段输入为本地 chunk。 |
