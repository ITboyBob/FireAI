...
| W：Word 提取策略 | `in_progress` | 真实签名、MIME、只读转换证据和双轴候选分类已实现；可信正文提取、边界和门禁仍未完成 | 真实签名探测、来源证据、内存候选、Word 专属门禁和 8 文件验收通过 | W-S1 |
...
| 批次发现与逐文件隔离 | `in_progress` | 已实现递归发现、稳定排序、来源 SHA-256 和冻结批次摘要；调度、汇总和逐文件提交尚未接入 | 批次只负责发现、调度和汇总；每份文件独立提交，失败互不污染 | W-S1 建立骨架，后续扩展 |
| 14 文件真实验收 | `in_progress` | 唯一 fixture 已与真实14文件核对，11份 Word 已完成只读真实分类；尚未端到端提交 | 各能力里程碑逐步覆盖；最终 14 份均有处理状态和可追溯证据 | 跨里程碑收口 |
