...
当前建设基线是：统一安全入口、上游策略路由、W 提取策略、S1/S2/S3 正文边界、法规语义质量资格和 Append-Only 提交已经完成实现与真实验收。W-S1、W-S2、W-S3 均已完成；Word/W 单文件 RAG 问答评测已进入设计与执行计划编写阶段，但尚无实现或真实评测证据；后续里程碑继续补充 PT、PS 和 S4 等尚未实现的策略能力。
...
| Word/W 单文件 RAG 问答评测 | `implemented` | 共享报告模型、不可变 dataset、真实 RAG Runner、独立 Judge、规则校验、聚合、目录级原子发布、主编排 CLI、运行配置与未校准声明、基线登记与同口径比较、产物核验器及对应自动化测试已全部落地；评测相关自动化测试 202 项通过。已完成 W-S1 `doc_d97773f1500c` 与 W-S2 `doc_0e84d13a099b` 共 6 题真实模型评测 Run `ws_rag_baseline_002_run_001`（修复问题 ID 跨文档重号后重建，question_id 全局唯一），产物经 `scripts/verify_ws_rag_evaluation_run.py` 核验通过，并登记 baseline `ws_rag_baseline_002`；旧 `ws_rag_baseline_001` dataset 与 run 产物已彻底清除，baseline `ws_rag_baseline_001` 经替换同样指向 `ws_rag_baseline_002_run_001` | 真实 Run 已生成、核验并登记 baseline；能力升级为 `implemented`（不代表 Judge 已获法律专家校准） | 建立并完成Word文档的评测系统 |
...
W-S1 里程碑本身不实现 PT、PS、S2、S3 的处理算法，也不让 S4 自动提交。W-S1 完成时，其他策略只登记能力状态、稳定接口和安全处置结果：未实现能力返回 `unsupported`，证据不充分返回 `review_required`，明确执行错误返回 `failed`；S2、S3 后续已由独立里程碑实现。
...
**Linear 规划状态：** 里程碑进度 100%，`BOB-20` 已完成

本里程碑已经在 Linear“消防AI”项目中创建为“建立并完成Word文档的评测系统”。当前唯一附属 issue `BOB-20` 负责完成评测系统设计文档和执行计划编写；代码实施与真实模型 Run 均已完成。
...
1. 以已完成的 W-S1/W-S2/W-S3 统一入口、公共主干和 S 类策略为复用基线；
2. 完成 Word/W 单文件 RAG 问答评测执行计划，再以 TDD 和真实 Word 文件证据推进实现；
3. 完成 PT 真实探针和提取策略，再复用已实现的 S 类策略；
4. PS 仅在 OCR 依赖获批并完成真实样本探针后进入实施；
5. 需要自动处理 S4 前，先设计并验收人工复核闭环；
...
| 2026-07-20 | 修复评测问题 ID 跨文档重号缺陷并重建基线：`generate_document_questions` 改为 dataset 级连续编号，`EvaluationDataset` 与发布链路补齐数据契约第 10 条 question_id 唯一性校验；新增 4 项回归测试；重新生成 dataset `ws_rag_baseline_002` 并完成真实 Run `ws_rag_baseline_002_run_001`（0/6 通过，`context_relevancy` 0.169，含 `missing_required_refusal` 红线），登记 baseline `ws_rag_baseline_002`；Dashboard 加载确认新 Run 可用、旧 Run 按契约判为不可用 | 4 项新回归测试先红后绿；`tests/eval_ws_rag` 202 项通过；完整回归 676 项通过；`scripts/verify_ws_rag_evaluation_run.py` 核验通过；`dashboard_loader.build_snapshot` 实测 valid/invalid 分类正确 |
| 2026-07-20 | 按用户指令彻底清除旧评测产物：删除 `data/eval/ws_rag_datasets/ws_rag_baseline_001/` 与 `reports/ws_rag_eval/ws_rag_baseline_001_run_001/`；baseline `ws_rag_baseline_001` 经 `register_baseline(replace=True)` 指向 `ws_rag_baseline_002_run_001`（保留 `previous_run_path` 审计轨迹）；当前 baseline 注册表两个条目均指向新 Run，Dashboard 仅剩新 Run 且零不可用报告 | `load_baseline(ws_rag_baseline_001)` 实测解析为新 Run；`build_snapshot` 实测 invalid 列表为空 |
