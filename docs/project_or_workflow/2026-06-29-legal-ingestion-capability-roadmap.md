...
当前建设基线是：统一安全入口、上游策略路由、W 提取策略、S1/S2/S3 正文边界、法规语义质量资格和 Append-Only 提交已经完成实现与真实验收。W-S1、W-S2、W-S3 均已完成；后续里程碑继续补充 PT、PS 和 S4 等尚未实现的策略能力。
...
2. 完成 PT 真实探针和提取策略，再复用已实现的 S 类策略；
3. PS 仅在 OCR 依赖获批并完成真实样本探针后进入实施；
4. 需要自动处理 S4 前，先设计并验收人工复核闭环；
