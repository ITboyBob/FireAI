...
- baseline `ws_rag_baseline_002` 已登记，dataset 与 run 产物已通过 `scripts/verify_ws_rag_evaluation_run.py` 核验；2026-07-20 发现并修复问题 ID 跨文档重号缺陷（生成器改为 dataset 级连续编号，dataset 模型与发布链路补齐数据契约第 10 条唯一性校验），旧 `ws_rag_baseline_001` 的 dataset 与 run 产物保留在磁盘但按契约判为不可用，不参与后续比较；
