...
当前代码已经登记 `ContentClass.S3`，但正文边界策略注册表仍将其解析为 `unsupported`。W 提取、统一编排、通用质量资格和 Append-Only 提交已经实现，W-S3 只应新增 S3 边界能力及对应组合验收。

设计批准不等于能力已实现。在 S3 策略、测试、质量门禁和真实验收全部完成前，路线图状态保持 `planned`。
...
| 当前错误 | 尾部材料被并入第二十九条，污染结构化条文和 chunks |

旧探针产生的 37 个 chunks 已包含污染，不能作为 W-S3 完成后的预期 chunk 数。实施阶段必须在取得 confirmed S3 边界后，根据干净的 29 条结构化正文重新冻结预期 chunk 数。
...
后续实现应新增独立 `S3BoundaryStrategy`，目标模块为：
...
新增 S3 门禁会改变质量规则语义，因此后续实现应将规则集从 `legal-quality-v2` 升级到 `legal-quality-v3`，并同步 S1、S2 回归断言。版本升级不表示 S1、S2 算法变化。
...
W-S3 里程碑实施计划已按“总览 + 2 个分卷”写入 [执行入口](../project_or_workflow/2026-07-04-s3-boundary-and-w-s3-acceptance-implementation.md)，下一阶段应从 Task 1 开始，而不是跳过计划直接编码：

| 文档 | 职责 |
| --- | --- |
| W-S3 实施计划总览 | 范围、依赖、全局门禁、Task 导航、提交策略和最终验收 |
| 分卷一：策略与质量能力 | 冻结范围、公共辅助函数无行为重构、S3 策略、注册接入、S3 门禁与规则版本升级 |
| 分卷二：真实验收与正式提交 | W-S3 dry-run、临时提交、回滚、正式导入、检索验收和文档收尾 |

计划 Task 顺序：

1. 冻结 W-S3 派生视图和真实验收事实；
2. 先用测试保护 S1/S2，再抽取必要的公共边界辅助函数；
3. TDD 实现独立 S3 策略；
4. 注册 S3 并接入统一编排器；
5. 增加 S3 专属门禁并升级 `legal-quality-v3`；
6. 执行真实文件 dry-run 并冻结干净 chunk 数；
7. 完成临时 Append-Only、失败隔离和检索回归；
8. 正式导入唯一 W-S3 文件，同步路线图与 Linear 里程碑。

实施计划必须逐 Task 明确：是否需要新增依赖、使用的包管理工具、阶段输入检查、预期输出、测试命令意图与预期结果，以及 Git commit 策略。未经实施计划明确授权，不进入代码实现。
...
- 当前 W-S3 实施计划只能把本文转换为可执行 Task，不得改写本文的边界语义。

## 16. 设计结论

S3 的安全实现不能等价为“遇到附件就截断”。系统必须先确认唯一且完整的法规正文，再以至少一个实质尾部强信号和一组辅助信号证明正文后的材料属于可排除区域。

W-S3 首个验收文件应得到 29 条干净正文；印发信息、附件目录、5 套文书模板和页码域只留下来源范围、类型和原因，不进入任何可检索产物。任何边界歧义都必须在 Append-Only 正式提交前停止。
