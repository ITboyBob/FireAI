...
本文档记录当前仓库仍有效的技术架构、目录边界、运行命令、建库规则、证据约束、错误处理、测试要求和前端工程组织标准。若后续实现改变这些工程规则，应同步更新本文档与[docs 文档索引](../CODEMAP.md)。
...
- `scripts/import_new_corpus.py` 是唯一薄 CLI，负责显式追加一个全新 `.doc/.docx` 法规文件或一份显式批次清单，不替代全量重建入口。
- 当前已实现 W 提取与 S1/S2/S3 边界策略；批次清单可通过 `--extraction-class` 与 `--content-class` 显式选择组合。PT、PS 和 S4 尚未形成自动提交能力。
- 显式批次按文件串行评估和独立提交，不提供 run 级原子事务；单个文件失败不得回滚同批次已成功提交的其他文件。
- 所有正式摄取必须经过统一编排器；正文边界状态必须为 `confirmed`、提交资格必须为 `qualified` 后才能进入 staging 和正式提交；`unsupported`、`review_required` 或 `failed` 状态均不得产生正式写入。
...
- 第一版增量导入不支持目录扫描、PDF/TXT/网页导入、旧法规修订替换或条文级 diff。显式批次只提供逐文件隔离和汇总，不承诺整批原子提交。
...
- 流开始后的 `error.code` 至少包括 `model_error`、`internal_error`、`conversation_not_found`、`persistence_error`；`conversation_not_found` 表示流中确认会话已被删除，`retryable` 为 `false`。`persistence_error` 仅当 `received` 已发出后的持久化步骤失败时使用，分瞬时档（可重试）与固有档（不可重试），`message` 分别为"服务内部异常，请稍后重试。"与"回答无法保存，可能需要管理员检查系统存储或服务状态"。`received` 发出前的失败仍按 `internal_error` 表达。前端以 `message` 文案呈现失败类型，并依据 `conversation_not_found` 执行离开失效会话 URL 的分支处理。
- 持久化失败分型的实现口径：仅对 `received` 之后的 4 个写方法（assistant 消息落库、turn 创建、answer snapshot 保存、history summary 保存）做异常封装，归一为 `PersistenceError(transient: bool)`；瞬时判定仅限 locked 类 `sqlite3.OperationalError`，其余数据库异常一律归固有档；快照一致性守卫抛出的 `RuntimeError` 显式归固有档；读路径不做封装。
...
- E2E 浏览器自动化基于 Playwright：运行前确认 `fire` 环境三件套就绪（`playwright`、`pytest-playwright`、Chromium）。
- 前端行为自动化验收使用 Playwright E2E：conda 环境 `fire` 内安装 `playwright`、`pytest-playwright` 并执行 `playwright install chromium`；E2E 通过 `page.route` mock `/api/conversations*` 的可控 NDJSON 与状态码，不依赖真实模型或索引；不得依赖 `waitForTimeout`；选择器使用现有 `data-testid`；`trace` 与 `screenshot` 仅在失败时保留。
## 前端工程组织
