...
- 查看评测运行产生的结构化错误；
- 在只有一个 Run 时明确提示尚不可比较。

本轮只使用一个符合共享 schema 的 mock Run，不读取或等待真实 Run。Dashboard 不启动评测、不修改阈值，也不重新计算评测结论。
...
- 总览、文件列表、问题下钻，以及“暂无可比较 Run”四类视图；
- 显式刷新；
- 缺失文件、损坏 JSON、schema 不兼容和跨字段不一致提示；
- 只维护一个合法 mock Run；执行单元测试、Streamlit `AppTest` 和基于该 mock Run 的浏览器烟雾验证。
...
- 真实 Run 核对、双 Run 对比实现、真实 baseline/current 差值和模型改进结论。
...
首轮只加载一个 mock Run。对比视图固定显示“暂无可比较 Run”，不渲染差值、涨跌箭头、改进/退化结论或文件修复状态。

双 Run 比较的数据兼容条件仍保留在数据契约中，作为未来扩展边界；它不属于本轮实施或验收范围。
...
5. 不得用单个 mock Run 输出任何“提升/下降”判断；
6. 不得把用户可见文本交给 `unsafe_allow_html=True`；
7. 服务默认绑定 `127.0.0.1`，不得默认暴露到局域网；
8. 依赖安装必须在 `fire` conda 环境中，经用户确认后使用 `python -m pip`；
9. 本轮完成状态只能记为 `mock_mvp`，路线图能力继续保持 `planned`；
10. 单元测试、`AppTest`、浏览器验证和单个 mock Run 字段核对全部通过后，才可完成本轮文档收口。
