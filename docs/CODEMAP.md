...
`docs/` 是按时间线归档的设计、实施与参考文档库：法规摄取架构与新评测系统等设计文档（architecture_or_strategy）、逐里程碑的 TDD 实施计划记录（project_or_workflow）、以及历史设计与运行时契约核验资料（reference_material）。
...
| --- | --- | --- | --- |
| `architecture_or_strategy/` | 架构设计文档 | 上位文档关系由各文「上位设计」字段自述；引用 `法律文本/todo/` 与 `data/` 结构 | 存放产品 PRD、工程技术标准、法规摄取专项设计与 ADR、新评测系统设计卷册（2026-04 → 2026-07/08） |
| `project_or_workflow/` | 项目实施记录 | 分别依赖 architecture_or_strategy 中对应设计文档 | 按 TDD 分批执行的各里程碑实施计划及其分卷存档（2.0 实施、增量导入、W-S1/S2/S3） |
| `reference_material/` | 参考资料档案 | 被现行 PRD / 工程技术标准在权威性上覆盖 | 一期历史设计、标准化切块重构、todo 语料评估基线和运行时契约核验记录 |
| `repo_overview/` | （目录不存在） | — | 当前不存在该子目录，预留为空概念 |
| `system_meta/` | （目录不存在） | — | 当前不存在该子目录；预留为空概念 |
